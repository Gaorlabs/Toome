...
export const Sidebar: React.FC<SidebarProps> = ({ currentView, onNavigate, collapsed, toggleCollapse, userRole, allowedModules, onLogout }) => {
  ...
  const analyticalItems = [
    ...
    { id: ViewMode.BRANCHES, label: 'Sedes y Cajas', icon: Store, module: 'SALES' as AppModule }, 
    { id: ViewMode.CUSTOMERS, label: 'Ventas Detalladas', icon: ShoppingCart, module: 'SALES' as AppModule },
  ...
  // Filter lists before rendering to check for empty sections
  const visibleAnalytical = analyticalItems.filter(i => isAllowed(i.module));
  const visibleOperations = operationsItems.filter(i => isAllowed(i.module));
